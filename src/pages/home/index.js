...
function Home() {
  ...
  const getApps = () => {
    setLoading(true)
    axios.get('https://sheltered-garden-95685.herokuapp.com/apps')
    .then((response) => {
      setLoading(false)
  ...
  const createApp = (name) => {
    axios.post('https://sheltered-garden-95685.herokuapp.com/apps', {name})
  ...
  const publishLayout = (layout) => {
    ...
    axios.post('https://sheltered-garden-95685.herokuapp.com/layout', {id: app.id, layout: JSON.stringify(layout)})
    .then(function (response) {
      publishing()
    ...
    <Layout style={{position: 'relative'}}>
      ...
      <Layout>
        <Sider>
            ...
            <Menu.ItemGroup title={t('apps')}>
              {
                apps.map((app) => (
                  <SubMenu key={`app_${app.id}`} title={app.name}>
                    ...
                    <Menu.Item key={`${app.id}#detail`}>{t('get_app_code')}</Menu.Item>
                  </SubMenu>
                ))
              }
          ...
          <Button type="primary" className="home-createapp" onClick={() => setCreateModal(true)} >{t('create_new_app')}</Button>
        </Sider>
        <Layout>
          <Content>
